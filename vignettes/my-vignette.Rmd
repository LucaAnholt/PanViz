...
Load the PanViz library into the _R_ session
...
snps <- PanViz::er_snp_vector
##creating an IMON using this summary-level GWAS data
IMON <- PanViz::get_IMON(snp_list = snps, ego = 5, save_file = FALSE)
print(IMON)
```
...
In the case that a summary-level associations tsv file was downloaded from the GWAS Catalog, users can use the GWAS_catalog_tsv_to_dataframe() function to parse the raw tsv file into an _R_ dataframe object. In this example, a tsv file containing the summary-level associations data for estrogen receptor-positive breast cancer (EFO_1000649) is used. 
...
IMON_D3 <- igraph_to_networkD3(IMON, group =  V(IMON)$col)
...
# Updating the PanViz database:

The underlying biochemical and metabolic reaction network database that PanViz uses to construct IMONs is queried from the Kyoto Encyclopedia of Genes and Genomes (KEGG) database (see Figure 1 and 2). As of this package being developed, the database was last pulled on the 15th January 2022. However, if users require the latest image of the KEGG database, we have included a convenience function in order to update the underlying PanViz databases. It should be noted that this can take a while in order to complete (upwards of 20 minutes) and the download speed is dependent on the user's own network speed and the KEGG RESTFUL API service. 

```{r, eval = FALSE}
##updating the underlying PanViz metabolic/reaction network database: 
