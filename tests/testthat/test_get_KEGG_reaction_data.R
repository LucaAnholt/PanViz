...
testthat::test_that("tests that get_KEGG_data returns list object and throws correct errors on bad inputs", {
  testthat::expect_error(PanViz::get_KEGG_reaction_data(CPU = 5, sleep = 5, progress_bar = TRUE), regexp = "Can only use a maximum of 2 workers")
  ...
  PanViz::get_KEGG_reaction_data(CPU = 1, sleep = 5, progress_bar = TRUE)
