...
GWAS_catalog_tsv_to_dataframe <- function(file){
  ...
  results <- file %>%
    ...
    dplyr::filter(stringr::str_detect(.data$SNPS, "rs")) %>%
    ##select SNPs, STUDY and DISEASE/TRAIT columns:
    dplyr::select(.data$SNPS, .data$STUDY, .data$`DISEASE/TRAIT`) %>%
    ##Rename these columns:
    dplyr::rename(snps = .data$SNPS, studies = .data$STUDY, traits = .data$`DISEASE/TRAIT`)
  return(results)
}
